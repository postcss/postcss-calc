...
console.log(
  `  out-of-scope: ${buckets['out-of-scope'].length}  (token types outside the calc subset)`
);
...
for (const [cat, c] of [...byCategory.entries()].sort((a, b) =>
  ...
  console.log(
    `  ${cat.padEnd(20)} ${String(c.inScope).padStart(3)} / ${c.outOfScope}`
  );
}
