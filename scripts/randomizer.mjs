...
import {
  ...
  astToCalc,
} from '../test/helpers/arbitraries.mjs';
...
const MODE = process.env.RANDOMIZER_MODE ?? 'complex';
...
function printStatus(stats, currentDepth) {
  const ms = Date.now() - stats.start;
  const buckets = [...stats.byBucket.entries()]
    .map(([k, v]) => `${k}=${v}`)
    .join(' ');
  const finds = `weThrew=${stats.weThrew} theyThrew=${stats.theyThrew} mismatch=${stats.mismatch}`;
...
const SMALL_INT_LEAF = fc
  .integer({ min: -9, max: 9 })
  .map((value) => ({ type: 'Num', value }));
const sign1 = fc.constantFrom(1, -1);
const complexAstArb = fc.memo((depth) => {
  ...
  const productOf = fc
    .array(fc.tuple(sign1, sub), { minLength: 2, maxLength: 6 })
    .map((pairs) =>
      mkProduct(pairs.map(([exp, node]) => ({ exponent: exp, node })))
    );
  const minMaxCall = fc
    .tuple(
    ...
    .map(([name, args]) => ({ type: 'Call', name, args }));
  const clampCall = fc.tuple(sub, sub, sub).map(([lo, val, hi]) => ({
    type: 'Call',
    name: 'clamp',
    args: [lo, val, hi],
  }));
  const unaryCall = fc
    ...
    .map(([name, arg]) => ({ type: 'Call', name, args: [arg] }));
  const binaryCall = fc
...
function generatorFor(depth) {
  switch (MODE) {
    case 'astArb':
      return astArb(depth);
    case 'astArbDegen':
      return astArbWithDegenerate(depth);
    case 'complex':
    default:
      return complexAstArb(depth);
  }
}
function recordOutcome(stats, outcome, depth, tokens, bucket, input) {
  switch (outcome.kind) {
    case 'agree':
      stats.agree++;
      return;
