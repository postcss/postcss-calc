...
const call = (name: string, args: Node[]): Node => ({ type: 'Call', name, args });
const num = (v: number): Num => ({ type: 'Num', value: v });
const dim = (v: number, u: string): Node => ({ type: 'Dim', value: v, unit: u });
const ident = (n: string): Node => ({ type: 'Ident', name: n });

// Finite, non-zero numeric leaf — domain for most laws.
const finiteNum = fc.integer({ min: -1000, max: 1000 }).map(num);
const finiteNonzeroNum = fc.integer({ min: -1000, max: 1000 })
  .filter((v) => v !== 0)
  .map(num);
const positiveNum = fc.integer({ min: 1, max: 1000 }).map(num);

// Same-unit dim pair (so foldConstArgs works without conversion noise).
const SAME_UNIT_DIMS = ['px', 'em', 'deg', 's', 'rem'] as const;
const finiteDim = fc
  .tuple(fc.integer({ min: -1000, max: 1000 }), fc.constantFrom(...SAME_UNIT_DIMS))
  .map(([v, u]): Dim => ({ type: 'Dim', value: v, unit: u }));

const finiteLeaf = fc.oneof<fc.Arbitrary<Num | Dim>[]>(finiteNum, finiteDim);
  ...
  fc.assert(
    fc.property(
      ...
      (strategy, x, b) => {
        ...
        const twice = out(call('round', [ident(strategy), num(parseFloat(once)), b]));
        return once === twice;
      }
    ),
    { numRuns: NUM_RUNS }
...
test('law: round to-zero ∈ {up, down} and minimizes |result|', () => {
  fc.assert(
    fc.property(finiteNum, positiveNum, (x, b) => {
      ...
      const minimal = Math.abs(tz) <= Math.abs(up) && Math.abs(tz) <= Math.abs(down);
...
const FLOAT_RANGE = { min: -1000, max: 1000, noNaN: true } as const;
...
test('law: tan(0) ≡ 0; atan(0) ≡ 0deg; atan(1) ≡ 45deg', () => {
  ...
  if (out(call('atan', [num(1)])) !== '45deg') throw new Error('atan(1) ≠ 45deg');
});
...
test('law: sqrt(pow(x, 2)) ≡ abs(x) for finite x', () => {
  fc.assert(
    fc.property(fc.integer({ min: -100, max: 100 }), (v) => {
      const lhs = parseFloat(out(call('sqrt', [call('pow', [num(v), num(2)])])));
...
test('law: exp(log(x)) ≡ x for finite x > 0 within precision', () => {
  fc.assert(
    fc.property(fc.float({ min: Math.fround(1e-3), max: Math.fround(1e6), noNaN: true }), (v) => {
