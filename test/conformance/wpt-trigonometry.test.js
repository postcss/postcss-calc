...
describe('WPT trigonometric functions with pi', () => {
  ...
  test('WPT trig-pi: cos(2 * pi) → 1', () => {
    assert.equal(out('cos(2 * pi)'), 'calc(1)');
  });

  test('WPT trig-pi: tan(pi / 4) → 1', () => {
    assert.equal(out('tan(pi / 4)'), 'calc(1)');
  });
});
...
describe('WPT inverse trigonometric functions', () => {
