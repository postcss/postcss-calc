...
describe('clamp()', () => {
  test('simplify: clamp folds when fully resolved', () => {
    ...
    assert.equal(out('clamp(0px, -5px, 10px)'), 'calc(0px)');
  ...
  test('simplify: clamp returns MIN when MIN > MAX (spec §10.8)', () => {
    ...
    assert.equal(out('clamp(5, -2, -9)'), 'calc(5)');
    assert.equal(out('clamp(0px, -2px, -1px)'), 'calc(0px)');
    assert.equal(out('clamp(128, 64, 3)'), 'calc(128)');
  });
  ...
  test('simplify: clamp with none keyword (spec §10.5.3)', () => {
    ...
    assert.equal(out('clamp(none, 50px, 20px)'), 'calc(20px)');
    ...
    assert.equal(out('clamp(10px, 20px, none)'), 'calc(20px)');
    assert.equal(out('clamp(30px, 20px, none)'), 'calc(30px)');
    ...
    assert.equal(out('clamp(none, 10px, none)'), 'calc(10px)');
    assert.equal(out('clamp(none, var(--x), none)'), 'var(--x)');
    assert.equal(out('clamp(none, 10px + 20px, none)'), 'calc(30px)');
    assert.equal(
