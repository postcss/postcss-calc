...
test('boundary: round at exact tie midpoints across signs', () => {
  ...
  assert.equal(out('round(5, 10)'), 'calc(10)'); // {0, 10}, tie → upper
  assert.equal(out('round(-5, 10)'), 'calc(0)'); // {-10, 0}, tie → upper (= 0)
  assert.equal(out('round(15, 10)'), 'calc(20)');
  assert.equal(out('round(-15, 10)'), 'calc(-10)');
  assert.equal(out('round(25, 10)'), 'calc(30)');
  assert.equal(out('round(0.5, 1)'), 'calc(1)');
  assert.equal(out('round(-0.5, 1)'), 'calc(0)');
});
...
describe('numeric boundaries', () => {
  test('boundary: round just-below-tie and just-above-tie', () => {
    ...
    assert.equal(out('round(4.9, 10)'), 'calc(0)');
  ...
  test('boundary: round on exact multiple preserves value', () => {
    assert.equal(out('round(20, 10)'), 'calc(20)');
    assert.equal(out('round(0, 10)'), 'calc(0)');
    assert.equal(out('round(-30, 10)'), 'calc(-30)');
    assert.equal(out('round(up, 20, 10)'), 'calc(20)');
    assert.equal(out('round(down, 20, 10)'), 'calc(20)');
    assert.equal(out('round(to-zero, 20, 10)'), 'calc(20)');
  });
  ...
  test('boundary: round with B much larger than A', () => {
    assert.equal(out('round(0.001, 1000)'), 'calc(0)');
    assert.equal(out('round(up, 0.001, 1000)'), 'calc(1000)');
    assert.equal(out('round(down, 0.001, 1000)'), 'calc(0)');
    assert.equal(out('round(to-zero, 0.001, 1000)'), 'calc(0)');
    assert.equal(out('round(to-zero, -0.001, 1000)'), 'calc(0)');
  });
  ...
  test('boundary: abs on -0 collapses to 0 (mkSum drop-zero)', () => {
    ...
    assert.equal(out('abs(calc(-0))'), 'calc(0)');
  });
  ...
  test('boundary: mod where A === B → 0', () => {
    ...
    assert.equal(out('mod(5, 5)'), 'calc(0)');
    assert.equal(out('mod(-5, -5)'), 'calc(0)');
    assert.equal(out('mod(5px, 5px)'), 'calc(0px)');
  });

  test('boundary: rem where A === B → 0', () => {
    assert.equal(out('rem(5, 5)'), 'calc(0)');
    assert.equal(out('rem(-5, -5)'), 'calc(0)');
  });
  ...
  test('boundary: dimensional 0 in mod/rem keeps unit', () => {
    ...
    assert.equal(out('rem(0px, 5px)'), 'calc(0px)');
    assert.equal(out('mod(0deg, 90deg)'), 'calc(0deg)');
  });
});
